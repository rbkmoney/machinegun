...
-spec write_files([file_contents()]) -> ok.
write_files(Files) ->
    ok = lists:foreach(fun write_file/1, Files).

-spec write_file(file_contents()) -> ok.
...
-spec filename(maybe(string())) -> maybe(filename()).
filename(Filename) when is_list(Filename) ->
    Filename;
filename(Filename) ->
    erlang:throw({bad_file_name, Filename}).

-spec file(maybe(string()), _AtMostMode :: non_neg_integer()) -> filename().
...
-spec mem_words(maybe(string())) -> maybe(mem_words()).
mem_words(undefined) ->
    undefined;
mem_words(MemStr) ->
    mem_bytes(MemStr) div erlang:system_info(wordsize).

-spec mem_bytes(maybe(string())) -> maybe(mem_bytes()).
mem_bytes(undefined) ->
    undefined;
mem_bytes(MemStr) ->
...
-spec rev_str_int(string(), Error :: term()) -> integer().
...
-spec time_interval(maybe(string())) -> maybe(time_interval()).
time_interval(undefined) ->
    undefined;
time_interval(TimeStr) ->
    parse_time_interval(TimeStr).

-spec time_interval(maybe(string()), time_interval_unit()) -> maybe(timeout()).
...
-spec parse_time_interval(string()) -> time_interval().
parse_time_interval(TimeStr) ->
...
-spec proplist(yaml_config() | undefined) -> proplists:proplist() | undefined.
proplist(undefined) ->
    undefined;
proplist(Config) ->
