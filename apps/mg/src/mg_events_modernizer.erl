...
update_event(Event = #{body := Body}, ModernizedBody) ->
    case Versions = {get_format_version(Body), get_format_version(ModernizedBody)} of
        ...
        {VersionWas, VersionWas} ->
            ...
            Event;
        _ ->
            % Обновлённое представление проверсионированных данных c такой же или более младшей версией,
            % или даже без неё. Это нарушение протокола, лучше вылететь с ошибкой?
